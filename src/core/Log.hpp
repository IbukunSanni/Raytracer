...
enum class Cat : int {
	RENDER = 0,   // the render loop, camera, sampling
	SCENE,        // scene graph construction and traversal
	GEOM,         // primitives, meshes, acceleration structures
	LUA,          // the binding layer and scene files
	IMAGE,        // framebuffer resolve and file output
	COUNT
};
...
public:
	...
	std::ostream & stream() { return m_buf; }
...
#define RT_LOG_AT(lvl, cat)                                                    \
	if ((int)(::rt::log::Level::lvl) > RT_LOG_LEVEL ||                         \
	    !::rt::log::enabled(::rt::log::Level::lvl, ::rt::log::Cat::cat)) {     \
	} else                                                                     \
		::rt::log::Line(::rt::log::Level::lvl, ::rt::log::Cat::cat).stream()

#define LOG_ERROR(cat) RT_LOG_AT(Error, cat)
#define LOG_WARN(cat)  RT_LOG_AT(Warn,  cat)
#define LOG_INFO(cat)  RT_LOG_AT(Info,  cat)
#define LOG_DEBUG(cat) RT_LOG_AT(Debug, cat)
