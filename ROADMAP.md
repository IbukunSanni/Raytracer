...
|---|---|---|---|
| `sampleUnitDisk()` | `Sampling.hpp` | returns (0,0), a pinhole | 5 |
| `thinLensRay()` | `Camera.hpp` | returns the pinhole ray | 5 |
| `AABB::hit()` | `AABB.hpp` | returns `true`, never culls | 8 |
| `BVH::build()` | `BVH.cpp` | leaves `m_built` false | 8 |
| `BVH::traverse()` | `BVH.cpp` | only called once built | 8 |
...
- [ ] **Resolution is capped at 920×891 and segfaults above it.** The
...
- [ ] **Portability: the code only builds under GCC.** MSVC fails on the `and`
      / `or` alternative tokens (`A4.cpp:108`, `Mesh.cpp:104`), on `uint` (a
      MinGW type, not standard C++), and on `polyroots.cpp` redefining `cbrt`.
      Worth fixing because Visual Studio's sampling profiler is the tool you
      need for step 8's writeup.
- [ ] **Latent: children of a `GeometryNode` are transformed twice.**
...
*Where you stand:* **scaffolded and ready.** `Sampling.hpp::sampleUnitDisk()`
and `Camera.hpp::thinLensRay()` are stubbed with the math spelled out;
...
`Camera.hpp`: quarter-disk sampling, world-axis offset instead of the camera
...
- [ ] **Background filename** is hardcoded to `"kh_stain_glass.png"` in
      ...
      light.
- [ ] **`RayTracer` is a `Ray`** — it holds an origin and a direction. Renaming
      it and making the getters `const` would make the whole `isHit` chain
      const-correct and remove the `const_cast` in `Sphere::isHit`.
...
- [ ] **Two different `EPS`** — `1e-6` in `A4.cpp`, `1e-5` in `Mesh.cpp`.
...
- [ ] **`premake4.lua`** is the old build system, redundant now CMake works.
- [ ] Root clutter: `sample.lua` sits at root while every other scene is in
      `Assets/`.
- [ ] `A4.cpp` / `A4.hpp` are the renderer now — the name is a leftover.
